...
const MemberMyEvents = () => {
  ...
  return (
    <div className="bg-dashboard-background dark:bg-background-dark text-dashboard-text-main dark:text-white font-display antialiased overflow-hidden h-screen flex">
      <MemberSidebar />
      
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-dashboard-background dark:bg-background-dark relative">
          ...
          <div className="max-w-6xl mx-auto flex flex-col gap-8">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 z-10 relative">
              <div className="flex flex-col gap-2">
                <h1 className="text-dashboard-text-main dark:text-white text-4xl font-bold leading-tight tracking-tight">My Events</h1>
                <p className="text-dashboard-text-muted dark:text-gray-400 text-base max-w-xl">
                  Track your upcoming schedule, manage registrations, and view your event history.
                </p>
              </div>
              <Link 
                to="/dashboard/member/discover"
                className="flex items-center gap-2 bg-dashboard-primary dark:bg-primary hover:bg-dashboard-primary-dark dark:hover:bg-primary-hover text-white px-6 py-3 rounded-full font-bold transition-transform active:scale-95 shadow-lg shadow-dashboard-primary/30 dark:shadow-primary/20"
              >
            ...
            <div className="flex flex-col gap-6 z-10 relative">
              ...
              <div className="relative w-full md:max-w-md">
                <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-dashboard-text-muted dark:text-gray-400">
                  <span className="material-symbols-outlined">search</span>
                </div>
                <input
                  className="w-full bg-dashboard-surface-hover dark:bg-surface-dark border border-dashboard-border dark:border-transparent focus:border-dashboard-primary dark:focus:border-primary/50 focus:ring-2 focus:ring-dashboard-primary/10 dark:focus:ring-primary/10 focus:bg-white dark:focus:bg-surface-dark text-dashboard-text-main dark:text-white rounded-full py-3 pl-12 pr-4 placeholder-dashboard-text-muted dark:placeholder-gray-500 font-medium transition-all"
              ...
              {/* Tabs */}
              <div className="border-b border-dashboard-border dark:border-white/10 w-full">
                  ...
                  <button
                    ...
                    className={`whitespace-nowrap pb-4 border-b-2 text-sm tracking-wide transition-colors ${
                      activeTab === 'my-joining'
                        ? 'border-dashboard-primary dark:border-primary text-dashboard-primary dark:text-white font-bold'
                        : 'border-transparent text-dashboard-text-muted dark:text-gray-400 hover:text-dashboard-text-main dark:hover:text-white font-medium hover:border-dashboard-border dark:hover:border-white/20'
                    }`}
                  >
                    Registered Events
                    <span className={`ml-2 text-xs py-0.5 px-2 rounded-full font-bold ${
                      activeTab === 'my-joining'
                        ? 'bg-dashboard-primary dark:bg-primary text-white'
                        : 'bg-dashboard-surface-hover dark:bg-surface-dark border border-dashboard-border dark:border-white/10 text-dashboard-text-muted dark:text-gray-400'
                    }`}>
                      {counts['my-joining'] || 0}
                    </span>
                  </button>
            ...
            {error ? (
              ...
              <div className="rounded-2xl border border-dashboard-border dark:border-white/10 bg-white dark:bg-surface-dark overflow-hidden shadow-sm dark:shadow-xl z-10 relative p-20 text-center">
                <div className="bg-dashboard-surface-hover dark:bg-surface-dark p-6 rounded-full mb-4 inline-block">
                  <span className="material-symbols-outlined text-6xl text-gray-400 dark:text-gray-600">event_busy</span>
                </div>
                <h3 className="text-xl font-bold text-dashboard-text-main dark:text-white">
                  {activeTab === 'my-joining' ? 'No registered events' : 
                   activeTab === 'upcoming' ? 'No upcoming events' :
                   activeTab === 'joined' ? 'No event history' :
                   'No cancelled events'}
                </h3>
                <p className="text-dashboard-text-muted dark:text-gray-400 mt-2 mb-6">
              ...
              <div className="rounded-2xl border border-dashboard-border dark:border-white/10 bg-white dark:bg-surface-dark overflow-hidden shadow-sm dark:shadow-xl z-10 relative">
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-white/5 border-b border-dashboard-border dark:border-white/10 text-dashboard-text-muted dark:text-gray-400 text-xs uppercase tracking-wider font-semibold">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-4">
                              ...
                              <div className="flex flex-col">
                                <span className="text-dashboard-text-main dark:text-white font-bold text-base group-hover:text-dashboard-primary dark:group-hover:text-primary transition-colors">
                                  {event.name}
                                </span>
                                <span className="text-dashboard-text-muted dark:text-gray-400 text-sm">{event.clubName}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex flex-col">
                              <span className="text-dashboard-text-main dark:text-white text-sm font-medium">{event.dateFormatted}</span>
                              <span className="text-dashboard-text-muted dark:text-gray-500 text-xs">{event.time}</span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-2 text-dashboard-text-muted dark:text-gray-300 text-sm">
                              <span className="material-symbols-outlined text-base text-dashboard-text-muted dark:text-gray-500">
                          ...
                          <td className="px-6 py-4 text-right">
                            {event.status === 'cancelled' ? (
                              <button 
                                ...
                                className="text-dashboard-text-main dark:text-white hover:text-red-400 text-sm font-semibold transition-colors"
                              >
                                View Details
                              </button>
                              ...
                              <button 
                                ...
                                className="text-dashboard-text-main dark:text-white hover:text-dashboard-primary dark:hover:text-primary text-sm font-semibold transition-colors"
                              >
                                Manage
                              </button>
                            ) : (
                ...
                {/* Pagination */}
                <div className="px-6 py-4 border-t border-dashboard-border dark:border-white/10 flex justify-between items-center text-sm text-dashboard-text-muted dark:text-gray-400">
